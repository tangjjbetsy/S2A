# Midi-VALLE: Midi Synthesize with VALLE model
1. We use the [Octuple Tokenisation](https://arxiv.org/abs/2106.05630) for tokenising Midis.
2. We applied embedding pooling to concate the embeddings for different features and input into the transformer.
3. All the other settings were kept as the same to the orignial implemtation. We tried prefix with 1, `exp/valle`,  and prefix with 2, `exp/valle-pre-2`.
4. For installing, please first follow the instruction of the below.
5. Please use the `egs/atepp/atepp.py` to prepare the data as the lhoste receipt.
6. Data and pre-trained models were saved in `egs/atepp/data.tar` and `egs/atepp/exp.tar`.
----------------------------------------------------------
Below is the original implementation of the [unoffical VALL-E](https://github.com/lifeiteng/vall-e)
----------------------------------------------------------
