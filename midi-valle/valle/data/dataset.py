...
from valle.data.collation import TextTokenCollater, MidiTokenCollater
...
class MidiSynthesisDataset(torch.utils.data.Dataset):
    """
    The PyTorch Dataset for the midi synthesis(e.g. TTS) task.
    Each item in this dataset is a dict of:
    ...
    .. code-block::

        {
            ...
            'audio_features': (B x NumFrames x NumFeatures) float tensor
            'audio_features_lens': (B, ) int tensor
            'midi_tokens': (B x NumTextTokens) long tensor
            'midi_tokens_lens': (B, ) int tensor
        }
    """

    def __init__(
        self,
        midi_token_collater: MidiTokenCollater,
        cut_transforms: List[Callable[[CutSet], CutSet]] = None,
        feature_input_strategy: BatchIO = PrecomputedFeatures(),
        feature_transforms: Union[Sequence[Callable], Callable] = None,
    ) -> None:
        ...
        self.midi_token_collater = midi_token_collater
        self.cut_transforms = ifnone(cut_transforms, [])
        self.feature_input_strategy = feature_input_strategy
    ...
    def __getitem__(self, cuts: CutSet) -> Dict[str, torch.Tensor]:
        ...
        return {
            "utt_id": [cut.id for cut in cuts],
            "midi": [cut.supervisions[0].text for cut in cuts],
            "audio": audio,
            "audio_lens": audio_lens,
            "audio_features": audio_features,
            "audio_features_lens": audio_features_lens,
            "midi_tokens": midi_tokens,
            "midi_tokens_lens": midi_tokens_lens,
        }
