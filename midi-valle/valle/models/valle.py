...
class VALLF(nn.Module):
    ...
    def __init__(
        ...
        vocab_sizes = [92, 68, 1156, 772, 388, 14]
        emb_sizes = [32, 32, 256, 128, 64, 32]
        
        self.feat_emb = []
        for i in range(6):
            self.feat_emb.append(TokenEmbedding(emb_sizes[i], vocab_sizes[i]))
        self.feat_emb = nn.ModuleList(self.feat_emb)
        
        self.ar_text_embedding = nn.Linear(sum(emb_sizes), d_model)
        self.nar_text_embedding = nn.Linear(sum(emb_sizes), nar_d_model)
    ...
    def forward(
        ...
        **kwargs,
        ...
        embs = []
        for i in range(6):
            embs.append(self.feat_emb[i](text[..., i]))
        embs = torch.cat([*embs], dim=-1)        
        #########
        
        x = self.ar_text_embedding(embs)
        ...
        if train_stage in [0, 2]:
            ...
            x = self.nar_text_embedding(embs)
